...
class ParseConfigTest(absltest.TestCase):
  ...
  def setUpClass(cls):
    ...
    cls.parse = parse_config.ParseConfig(
        ontology=cls.ontology, solution=cls.solution, proposed=cls.proposed
    )
  ...
  def testInitialize(self):
    ...
    self.assertEqual(
        type(self.parse.deserialized_files[PROPOSED]), dict
    )  # DeserializedFile
    self.assertEqual(
        type(self.parse.deserialized_files[SOLUTION]), dict
    )  # DeserializedFile
  ...
  def testAppendTypes(self, mock_print):
    ...
    self.assertTrue(
        hasattr(list(deserialized_files_appended[PROPOSED].values())[0], 'type')
    )  # pylint: disable=unsubscriptable-object

  def testRetrieveReportingTranslations(self):
    proposed_entities = validator.Deserialize(
        ['tests/samples/proposed/retrieve_reporting_translations.yaml']
    )[0]
    solution_entities = validator.Deserialize(
        ['tests/samples/solution/retrieve_reporting_translations.yaml']
    )[0]
    ...
    with patch.object(Dimension, 'is_entity_canonical', return_value=True):
      ...
      self.assertEqual(
          type(translations[cdid][f'{PROPOSED}'][0][1]), NonDimensionalValue
      )
      ...
      self.assertEqual(
          type(translations[cdid][f'{SOLUTION}'][0][1]), NonDimensionalValue
      )
  ...
  def testAggregateResults(self):
    ...
    self.assertEqual(
        results['_MockDimensionSimple'],
        'called with argument for simple dimensions',
    )
    self.assertEqual(
        results['_MockDimensionComplex'],
