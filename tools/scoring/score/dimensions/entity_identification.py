...
from score.scorer_types import CloudDeviceId, DeserializedFile
...
class EntityIdentification(Dimension):
  ...
  def evaluate(self):
    ...
    solution_reporting_ids = self._list_ids_reporting(
        file=solution_file, exclude_noncanonical=True
    )

    proposed_virtual_ids = self._list_ids_virtual(
        file=proposed_file, exclude_noncanonical=False
    )
    solution_virtual_ids = self._list_ids_virtual(
        file=solution_file, exclude_noncanonical=True
    )

    self.correct_reporting = sum(
        (
            Counter(proposed_reporting_ids) & Counter(solution_reporting_ids)
        ).values()
    )
    ...
    self.correct_virtual = sum(
        (Counter(proposed_virtual_ids) & Counter(solution_virtual_ids)).values()
    )
