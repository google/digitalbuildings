...
class OntologyWrapper(object):
  ...
  def _CalculateMatchScore(
    ...
    total_fields = len(concrete_fields.union(standard_canonical_fields))
    ...
    raw_score = matched_fields / total_fields
    if total_required_type_fields > 0:
      raw_score += (matched_required_fields / total_required_type_fields)
    return int((raw_score / 2.0) * 100.0)
  ...
  def GetEntityTypesFromFields(
    ...
    """
    if True not in [self.IsFieldValid(field) for field in field_list]:
      raise ValueError('List of fields contains only invalid fields.')
    entity_type_list = []
