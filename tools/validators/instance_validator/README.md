...
3. After a building configuration's entity types are validated, validation must also be run on the telemetry payload using:
  ...
  * `--udmi` **[Optional]** Validates entity metadata as [UDMI](https://github.com/faucetsdn/udmi/). Include argument when `True` and do not include when `False`.
  `python instance_validator.py -i input.yaml --udmi` validates a building config against the udmi standard.
