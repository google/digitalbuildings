...
class DefinedField(FieldTranslation):
  """A base class for translation of a single field that is present in the device payload.
...
class DimensionalValue(DefinedField):
  ...
  Attributes:
    ...
    raw_field_name: string. Fully qualified json path to the field in the device
      payload.
    unit_field_name: string. Fully qualified json path ot the unit in the device
      payload.
