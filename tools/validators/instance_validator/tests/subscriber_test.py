...
class SubscriberTest(absltest.TestCase):
  ...
  @mock.patch.object(subscriber, 'auth')
  @mock.patch.object(subscriber, 'pubsub_v1')
  def test_listen_no_service_account(self, test_pubsub, test_auth):
    test_auth.default.return_value = ('fake_credentials', 'fake_project_id')
    test_pubsub.SubscriberClient.return_value = None
    with self.assertRaises(AttributeError):
      self.subscriber.Listen(callback=mock_callback)
    test_auth.default.assert_called_once()
