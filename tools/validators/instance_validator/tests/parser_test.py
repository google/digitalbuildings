...
"""Tests tools.validators.instance_validator.validate.parser."""
...
class ParserTest(absltest.TestCase):
  ...
  def testDeserialize_DuplicateKeys_Fails(self):
    bad_testcase_path = [
      os.path.join(os.path.abspath('./fake_instances'), 'BAD',
                   'duplicate_keys.yaml')]

    entities, config_mode = self.parser.Deserialize(
      yaml_files=bad_testcase_path)
  ...
  def testDeserialize_BadSpacing_Fails(self):
    bad_testcase_path = [
      os.path.join(os.path.abspath('./fake_instances'), 'BAD', 'spacing.yaml')]

    with self.assertRaises(yaml.scanner.ScannerError):
      self.parser.Deserialize(yaml_files=bad_testcase_path)

  def testDeserialize_BadTabbing_Fails(self):
    bad_testcase_path = [
      os.path.join(os.path.abspath('./fake_instances'), 'BAD', 'tabbing.yaml')]
  ...
  def testDeserialize_BuildingConnections_Success(self):
    good_testcase_path = [
      os.path.join(os.path.abspath('./fake_instances'), 'GOOD',
                   'building_connections.yaml')]

    entities, config_mode = self.parser.Deserialize(
      yaml_files=good_testcase_path)
  ...
  def testDeserialize_BadTranslationNoCDID_Fails(self):
    ...
    self.assertNotIn(uuid.UUID('eb15ee68-795f-430a-bb1f-8e70eaf2e66a'),
                     entities)

  def testDeserializer_UpdateMaskWithoutUpdateOperation_Fails(self):
    bad_testcase_path = [
      os.path.join(os.path.abspath('./fake_instances'), 'BAD',
                   'update_mask_value.yaml')]
  ...
  def testDeserialize_DefaultExportOperationForUpdateMode_Success(self):
    good_testcase_path = [
      os.path.join(os.path.abspath('./fake_instances'), 'GOOD',
                   'entity_export_operation.yaml')]

    entities, config_mode = self.parser.Deserialize(
      yaml_files=good_testcase_path)

    self.assertEqual(config_mode, enumerations.ConfigMode.UPDATE)
    self.assertEqual(self.parser.GetDefaultEntityOperation(),
                     enumerations.EntityOperation.EXPORT)
    self.assertLen(entities, 1)
    self.assertIn(uuid.UUID('71965f14-9690-4218-9c9e-cb9550b8a07f'), entities)
