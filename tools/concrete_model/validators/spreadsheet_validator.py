...
from google3.third_party.digitalbuildings.tools.concrete_model.validators.spreadsheet_error import SpreadsheetError
...
class SpreadsheetValidator(object):
  ...
  def _ValidateContents(
      ...
      col_headers_values: List[str]) -> List[SpreadsheetError]:
        ...
        if row[source_column_header] not in (
          ...
          validation_errors.append(
              SpreadsheetError(
                  ...
                  message=f'{row[source_column_header]} must be present in {target_table} - {target_column_header}'
              ))
    return validation_errors

  def _ValidateHeaders(self, table: str, parsed_sheet: List[Dict[str, str]],
                       column_headers: List[str]) -> List[SpreadsheetError]:
    ...
    if header_difference:
      validation_errors.append(
          SpreadsheetError(
              table=table,
              row=_COLUMN_HEADER_INDEX,
              column='All Column Headers',
              message=f'{table} missing column headers: {header_difference}'))
    return validation_errors
  ...
  def _ValidateConnections(
    ...
    for row_number, connection in enumerate(connections_sheet,
      ...
      if connection[SOURCE_ENTITY_CODE] not in codes:
        message = f'{connection}{connection[SOURCE_ENTITY_CODE]} has a  {ENTITIES} or {SITES}'
        validation_errors.append(
            SpreadsheetError(
                table=CONNECTIONS,
                row=row_number,
                column=SOURCE_ENTITY_CODE,
                message=message))
      if connection[TARGET_ENTITY_CODE] not in codes:
        message = f'{connection[TARGET_ENTITY_CODE]} must be present in {ENTITIES} or {SITES}'
        validation_errors.append(
            SpreadsheetError(
                table=CONNECTIONS,
                row=row_number,
                column=TARGET_ENTITY_CODE,
                message=message))
    return validation_errors

  def _LogErrors(self, validation_errors: List[SpreadsheetError]) -> None:
    logging.basicConfig(
