...
class Entity(object):
  ...
  Attributes:
    ...
    etag: Hash used for PhRED entity versioning.
    namespace: DBO namespace for an entity i.e. HVAC.
  ...
  def AddConnection(self, new_connection: Connection) -> None:
    ...
    if new_connection.source_entity_guid != self.bc_guid:
      raise AttributeError(
          f'{new_connection.source_entity_guid} does not equal {self.bc_guid}')
...
class VirtualEntity(Entity):
  ...
  def __init__(self,
    ...
    super().__init__(code, namespace, etag, type_name, bc_guid,
                     metadata)
    self._links = []
...
class ReportingEntity(Entity):
  ...
  Attributes:
    ...
    translations: List of EntityField modeling datapoints on a reporting entity.
    udmi_guid: A compressed IFC GUID i.e. ifc://2mrlLbCg557vzSychQ81OI.
  """
  ...
  def __init__(self,
    ...
    Args:
      code: The entity's name.
      namespace: An entity's DBO namespace i.e. HVAC.
      udmi_guid: A GUID according to UDMI specifications i.e.
        ifc://2mrlLbCg557vzSychQ81OI.
    ...
    self.udmi_guid = udmi_guid
    self._translations = []

  def __eq__(self, other: ...) -> bool:
    if not isinstance(other, Entity):
      raise TypeError(f'{str(other)} must be an Entity instance')
    return (self.code, self.cloud_device_id, self.bc_guid,
            self.udmi_guid) == (other.code, other.cloud_device_id,
                                other.bc_guid, other.udmi_guid)
  ...
  def FromDict(cls, entity_dict: Dict[str, str]) ->...:
    ...
    reporting_entity_instance = cls(
        ...
        namespace=entity_dict[NAMESPACE],
        type_name=entity_dict[TYPE_NAME],
        udmi_guid=entity_dict[UDMI_GUID],
