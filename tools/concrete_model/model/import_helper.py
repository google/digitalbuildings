...
from google3.third_party.digitalbuildings.tools.concrete_model.model.constants import SITE_TYPE_NAME
...
def GetAllSheets(spreadsheet_id: str, spreadsheet_range: List[str],
                 google_sheets_service: Resource) -> Dict[str, object]:
...
def _ImportSheetFromGoogleSheets(
    spreadsheet_id: str, named_range: str,
    google_sheets_service: Resource) -> List[Dict[str, str]]:
  ...
  Raises:
    HttpError: If the Google Sheets API request returns an error.
  """
  spreadsheet_rows = []
  try:
