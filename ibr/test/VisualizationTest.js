...
import {Visualization} from '../ibr-sdk/class/Visualization.js';
...
test('should convert coordinates indices to Int32 Array', (t) => {
  t.deepEqual(t.context.visualization.getCoordinatesIndices(),
      new Uint32Array( [0, 2076] ));
});
...
test('should have encoding type recorded', (t) => {
  t.deepEqual(t.context.visualization.getEncodingType(),
      t.context.jsonData1.encoding_type );
});

test('should have image data recorded', (t) => {
  t.deepEqual(t.context.visualization.getImageData(),
      t.context.jsonData1.image_data );
});
