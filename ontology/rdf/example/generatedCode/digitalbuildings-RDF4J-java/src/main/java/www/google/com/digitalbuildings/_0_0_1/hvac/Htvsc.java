...
/**
* Class Htvsc 
* Variable gas to electric control on supply air side.
*/
@SuppressWarnings("serial")
public class Htvsc extends www.google.com.digitalbuildings._0_0_1.Control implements IHtvsc{

	IRI newInstance;
	public Htvsc(String namespace, String instanceId) {
		super(namespace, instanceId);

		newInstance = GLOBAL.factory.createIRI(namespace, instanceId);
		GLOBAL.model.add(this, RDF.TYPE, GLOBAL.factory.createIRI("http://www.google.com/digitalbuildings/0.0.1/hvac#Htvsc"));
	}
  ...
  public void addUsesHeater_run_status (IHeater_run_status parameter)
	{
		GLOBAL.model.add(this, GLOBAL.factory.createIRI("http://www.google.com/digitalbuildings/0.0.1#uses"), parameter);
	}

	public Set<IHeater_run_status> getUsesHeater_run_status (){
		Set<IHeater_run_status> UsesHeater_run_status = new HashSet<>();
		GLOBAL.model.filter(this, GLOBAL.factory.createIRI("http://www.google.com/digitalbuildings/0.0.1#uses"), null)
		.objects().forEach(action->{
			if(action instanceof Heater_run_status) {
				UsesHeater_run_status.add((Heater_run_status)action);
			}
		});
		return UsesHeater_run_status;
	}


  public void addUsesHeating_percentage_command (IHeating_percentage_command parameter)
	{
		GLOBAL.model.add(this, GLOBAL.factory.createIRI("http://www.google.com/digitalbuildings/0.0.1#uses"), parameter);
	}

	public Set<IHeating_percentage_command> getUsesHeating_percentage_command (){
		Set<IHeating_percentage_command> UsesHeating_percentage_command = new HashSet<>();
		GLOBAL.model.filter(this, GLOBAL.factory.createIRI("http://www.google.com/digitalbuildings/0.0.1#uses"), null)
		.objects().forEach(action->{
			if(action instanceof Heating_percentage_command) {
				UsesHeating_percentage_command.add((Heating_percentage_command)action);
			}
		});
		return UsesHeating_percentage_command;
	}
  ...
  public void addUsesOptionalsHeating_thermal_power_capacity (IHeating_thermal_power_capacity parameter)
	...
	public static Set<IHtvsc> getAllHtvscsObjectsCreated(){
		Set<IHtvsc> objects = new HashSet<>();
		objects = GLOBAL.model.filter(null, RDF.TYPE, GLOBAL.factory
		.createIRI("http://www.google.com/digitalbuildings/0.0.1/hvac#Htvsc")).subjects().stream()
		.map(mapper->(IHtvsc)mapper)
