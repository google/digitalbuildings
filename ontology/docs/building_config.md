...
rtegistration). These should be relatively obvious to a critical reader, but if
they are confusing feel free to post an issue in the project.

*   For an explanation of types in the Digital Buildings concrete model see
    [model](model.md)
...
*   **Virtual Entity** A virtual entity does NOT directly generate telemtry
    data, but may still have a timeseries that is generated by linking data from
    other sources.

The following statements applly when considering entity categories:
...
*   **Link**: A mapping between a standard field in one device and another
    ...
    virtual devices.
*   **Connection**: Describes a relationship berween two entities. See
...
logical devision of your universe). The reson for this is that the definitions
in the file are intentianally not keyed by a globally unique identifier. This is
done for a few of reasons:

1.  The human-readable entity name is, well, more human redable.
2.  Using human-readable names, which are typically created at design time and
    ...
    stages of the commissioning or onboarding process, rather than hacing to
...
*   **Entity Name:** The humand readable identifier for the entity. This should
...
*   **Connections:** Used to specify connections from other entities (sources)
    ...
    be repreated. Values are one or more connections, specified as a single
    value or a set.
...
the entity in your system. In the case of deevices sending data via Cloud IoT we
use the CDM `numId` prefixed with `CDM/` as a namespace.

Choose an entitiy type that has the correct fields for this type.
...
differ from our modeled entity types. In this case, it is necessaty to define a
...
In order to elimnate duplicate work, the format provides some shortcuts to
...
entities. System and spactial relationships are defined with the `connections`
...
simply virtual devices, following al lthe same rules, but often do not have
`links`.
...
System and spactial relationships are defined with the `connections` block.
...
## Validation (Coming Soon) {#validation}

The building config can be machine validated for consistency and adherance to
