...
"""Helpers for use in tests. DON NOT USE IN PRODUCTION CODE.
"""
...
def Field(fq_name, optional):
  ...
  Returns:
    a FieldParts tuple represting the field.
  """
  parts = fq_name.split('/')
  namespace = ''
  if len(parts) == 2:
  ...
  return entity_type_lib.OptWrapper(field=entity_type_lib.FieldParts(
      namespace=namespace, field=field, increment=increment), optional=optional)


def Fields(fq_fields):
  """Creates a qualified-field:tuple map for the list of qualified names.

  Args:
    fq_fields: an iterable of fully-qualified field names
  Returns:
    a map of fully qualified name to FieldParts tuple representing the list
  """
